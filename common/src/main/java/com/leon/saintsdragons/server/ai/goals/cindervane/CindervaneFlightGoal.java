...
public class CindervaneFlightGoal extends Goal {
    ...
    private boolean isWideLandingSurface(BlockPos ground) {
        ...
        // Check a 3x3 area to ensure the dragon's full hitbox has ground support
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                BlockPos checkPos = ground.offset(dx, 0, dz);
                var state = amphithere.level().getBlockState(checkPos);

                // All positions must be solid and not fluid
                if (state.isAir() || !state.getFluidState().isEmpty()) {
                    return false;
                }

                // Must have a sturdy top face
                if (!state.isFaceSturdy(amphithere.level(), checkPos, Direction.UP)) {
                    return false;
                }
            }
        }

        // Also check that the center has clear space above
