...
/**
 * NeoForge-specific part manager for Ignivorus hitbox parts.
 ...
 * - Tail: 0.8x (reduced)
 */
public class NeoForgeIgnivorusPartManager {
    private final Ignivorus dragon;
    private final Map<String, PartConfig> partConfigs = new LinkedHashMap<>();
    private final Map<String, NeoForgeDragonPart> parts = new LinkedHashMap<>();
    private NeoForgeDragonPart[] partsArray;

    // How far each hitbox stretches toward its neighbor (0.75 = 75% of the way)
    private static final float STRETCH_FACTOR = 0.75f;

    public NeoForgeIgnivorusPartManager(Ignivorus dragon) {
        this.dragon = dragon;
        initializePartConfigs();
        initializeParts();
        this.partsArray = parts.values().toArray(new NeoForgeDragonPart[0]);
    }
    ...
    private void initializeParts() {
        for (Map.Entry<String, PartConfig> entry : partConfigs.entrySet()) {
            ...
            NeoForgeDragonPart part = new NeoForgeDragonPart(dragon, partName, config.width(), config.height());
    ...
    public void updatePartPositions() {
        ...
        for (Map.Entry<String, NeoForgeDragonPart> entry : parts.entrySet()) {
            String partName = entry.getKey();
            NeoForgeDragonPart part = entry.getValue();
